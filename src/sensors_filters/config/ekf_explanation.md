...
**Matriz P (Initial Estimate Covariance):** Definida como 1e-9 (default) para as todas as variáveis, representando uma incerteza inicial praticamente nula. Isto porque, em simulação tenho total certeza que o robô começa em (0,0) com orientação 0° e velocidade nula. Para as variáveis 3D e acelerações inativas, utilizei 1e-9 (default) para que o filtro parta com grande confiança de que esses estados são zero.

**Matriz Q (Process Noise Covariance):** Os valores de Q(x,y) foram reduzidos para 0.01 (abaixo do default 0.05), e setados como default nas restantes variáveis. Um valor de Q menor indica que o modelo de predição cinemático é considerado mais fiável. Isto garante que o EKF minimize a confiança na leitura instantânea da odometria para a posição absoluta, atenuando o impacto de erros introduzidos por derrapagem das rodas (drift).