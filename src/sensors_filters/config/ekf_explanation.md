...
**Matriz Q (Process Noise Covariance):** Valores ajustados para 0.01 nos componentes de posição (x, y) e 0.05 nos restantes. A redução da incerteza do processo em **x** e **y** aumenta a rigidez matemática do modelo cinemático. Isto garantir que o EKF minimize a confiança na leitura instantânea da odometria para a posição absoluta, atenuando o impacto de erros introduzidos por derrapagem das rodas (drift). Contudo, a sua utilização é necessária para ajudar o filtro a ter uma referência contínua da sua posição.